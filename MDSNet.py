...
#==============================================================================
    ...
    Net = nn.Sequential()
    Net.add_module('Layer1',nn.Linear(3,HiddenLayer,bias = False))
    ...
    Net.add_module('LayerEnd',nn.Linear(HiddenLayer,2,bias = False))
...
#    Net = nn.Sequential()
#    Net.add_module('Layer1',nn.Linear(3,HiddenLayer,bias = False))
#    Net.add_module('NonLinear1',torch.nn.ReLU())
#
#    for layerIter in np.arange(2,NumberOfHiddenLayers+1):
#        Net.add_module('Layer'+str(layerIter),nn.Linear(HiddenLayer,HiddenLayer,bias = False))
#        Net.add_module('NonLinear'+str(layerIter),torch.nn.ReLU())
# 
#    Net.add_module('LayerEnd',nn.Linear(HiddenLayer,2,bias = False))
#
#    Numiter=LearningParams['Numiter']
...
#    tol = LearningParams['tol']
#    
#    optimizer = torch.optim.Adam(Net.parameters(), lr=learning_rate,  betas=(0.95, 0.999))
...
#        if (itern>0) and (np.abs(train_err[itern]-train_err[itern-1])<tol):
#           break;
